...
public class Wizard
{
        ...
        switch(state)
        {
            case 0:
                ...
                for (int i = 0; i < gameMaster.monsters.size(); i++) {
                    System.out.printf("%d. 이름: %s 체력: %d\t", i + 1, gameMaster.monsters.get(i).GetName(), gameMaster.monsters.get(i).GetHp());
                    System.out.println();
                }
                state = 1;
                break;
    ...
    public void MagicAttack(Monster monster)
    {
        ...
        {
            System.out.printf("가지고 있는 MP에 비해 요구량이 큰 공격을 사용하여 %s을(를) 사용합니다.",this.actions[0]);
            OriginAttack(monster);
        }
